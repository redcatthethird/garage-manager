...
<table border="1">
		...
		<tr><th>Plate Number</th><td><a href="{{ URL::route('cars.show', array($repair['LicencePlate'])) }}">{{ $repair['LicencePlate'] }}</a></td></tr>
		<tr><th>Staff in charge</th><td><a href="{{ URL::route('staff.show', array($repair['StaffID'])) }}">{{ $repair['StaffID'] }}</a></td></tr>
