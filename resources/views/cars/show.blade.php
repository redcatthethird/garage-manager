...
<h1>Repairs on car {{ $car->LicencePlate }}</h1>
		...
		<a href="{{ URL::route('cars.show', array($car['LicencePlate'])) }}">{{ $car['LicencePlate'] }}</a>
		<a href="{{ URL::route('clients.show', array($car['ClientId'])) }}">{{ $car['ClientId'] }}</a>
		{{$car['Model']}}</td>

		@if (Auth::user()->isAdmin)
			<a href="{{ URL::route('cars.edit', array($car['LicencePlate'])) }}">Edit</a>
...
<table border="1">
	...
	<tr>
		<td><a href="{{ URL::route('repairs.show', array($repair['Id'])) }}">{{$repair['Id']}}</a></td>
